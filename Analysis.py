...
if __name__ == '__main__':
    ...
    y1 = np.exp(-(x - 100) ** 2 / (2 * 10 ** 2)) / (np.sqrt(2 * np.pi) * 10)
    y2 = np.exp(-(x - 150) ** 2 / (2 * 20 ** 2)) / (np.sqrt(2 * np.pi) * 20)
    mu = (100/100+150/400)/(1/100+1/400)
    var = np.sqrt(1/(1/100+1/400))
    y3 = np.exp(-(x - mu) ** 2 / (2 * var ** 2)) / (np.sqrt(2 * np.pi) * var)
    plt.plot(x, y1, label="Gaussian 1")
    plt.plot(x, y2, label="Gaussian 2")
    plt.plot(x, y3, label="Product_Gaussian")
    plt.legend()
    plt.xlabel("x")
    plt.ylabel("y")
    plt.savefig("../myfigure/Gaussians", dpi=300)
    plt.show()
