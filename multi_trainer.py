...
from ray.rllib.agents.ppo.ppo_torch_policy import kl_and_loss_stats, \
    vf_preds_fetches, setup_mixins, KLCoeffMixin
...
def compute_gae_for_sample_batch(
    ...
    if not sample_batch[SampleBatch.INFOS].dtype == "float32":
        ...
        samplebatch_infos_rewards = SampleBatch.concat_samples([
            SampleBatch({str(i): [v] for i, (k, v) in enumerate(s.items())})
...
def ppo_surrogate_loss(
    ...
    mean_kl = reduce_mean_valid(torch.mean(action_kl, axis=1))
    ...
    model.tower_stats["total_loss"] = torch.mean(
